...
export default function DonationWidget({ campaign }) {
  ...
  const handleDonate = async () => {
    ...
    try {
      // Field names must match /donations/initiate.php:
      // campaign_id, name, email, amount
      const payload = {
        campaign_id: campaign.id,
        name: anonymous ? "Anonymous" : (name.trim() || "Anonymous"),
        email: email.trim() || "donor@trustbridge.ng",
        amount: parsedAmount,
      };
      ...
      // Page navigates away — no need to reset isDonating
    } catch (err) {
      console.error("Donation init failed:", err);
      const msg =
