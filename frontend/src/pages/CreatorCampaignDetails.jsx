...
import { getMyCampaignDetails, deleteCampaignCreator } from "@/lib/api";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function CreatorCampaignDetails() {
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await deleteCampaignCreator(id);
  ...
  // Pluck the image paths effectively so the ImageGallery component works normally
  const imageStrings = rawImages.map((img) => img.image_path);
  ...
  // Generate the public share URL instead of the current private "/creator/campaigns/:id" URL
  const shareUrl =
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid lg:grid-cols-5 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
              ...
              <div className="flex items-end gap-2 mb-2">
                ...
                <span className="text-slate-500 mb-1">
                  raised of ₦{(campaign.target_amount || 0).toLocaleString()}
                </span>
              </div>

              <div className="h-2 w-full bg-slate-100 rounded-full mt-4 mb-4 overflow-hidden">
                <div
              ...
              <div className="grid grid-cols-2 gap-4 my-6">
                <div className="bg-slate-50 p-4 rounded-2xl">
                  <p className="text-slate-500 text-sm mb-1">Total Donors</p>
                  <p className="font-bold text-xl text-slate-800">
                    {stats?.total_donors || 0}
                  </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl">
                  <p className="text-slate-500 text-sm mb-1">Donations</p>
                  <p className="font-bold text-xl text-slate-800">
            ...
            <div className="bg-white rounded-3xl p-6 border border-red-50/50 shadow-sm sticky top-6">
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex items-center gap-3 p-3 bg-red-50 text-red-700 rounded-lg text-sm mb-4">
            ...
            <p>
              Deleting: <strong>{campaign?.title}</strong>
            </p>
          </div>
